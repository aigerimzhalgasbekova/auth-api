...
describe('AuthApiInfrastructure', () => {
  ...
  describe('TokenIssuerStack', () => {
    ...
    test('creates token issuer Lambda function', () => {
      template.hasResourceProperties('AWS::Lambda::Function', {
        ...
        VpcConfig: {
          SecurityGroupIds: [
            Match.objectLike({ 'Fn::ImportValue': Match.stringLikeRegexp('SecurityGroup') }),
          ],
          SubnetIds: [
            Match.objectLike({ 'Fn::ImportValue': Match.stringLikeRegexp('Subnet1') }),
            Match.objectLike({ 'Fn::ImportValue': Match.stringLikeRegexp('Subnet2') }),
          ],
        },
      });
    });
  ...
  describe('EchoApiStack', () => {
    ...
    beforeEach(() => {
      const networkStack = new NetworkStack(app, 'TestNetworkStack');
      const tokenIssuerStack = new TokenIssuerStack(app, 'TestTokenIssuerStack', {
        vpc: networkStack.vpc,
        securityGroup: networkStack.lambdaSecurityGroup,
        signingKeyAlias: 'alias/signing-key',
        artifactBucketName: 'artifact-bucket',
    ...
    test('creates echo Lambda function', () => {
      template.hasResourceProperties('AWS::Lambda::Function', {
        ...
        VpcConfig: {
          SecurityGroupIds: [
            {
              'Fn::ImportValue': Match.stringLikeRegexp('SecurityGroup'),
            },
          ],
          SubnetIds: [
            {
              'Fn::ImportValue': Match.stringLikeRegexp('Subnet1'),
            },
            {
              'Fn::ImportValue': Match.stringLikeRegexp('Subnet2'),
            },
          ],
        },
      });
